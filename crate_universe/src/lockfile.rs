...
impl Digest {
    ...
    fn compute(
        ...
        // Create a normalized version of splicing metadata for hashing.
        // The SplicingMetadata contains Labels with workspace-specific package paths
        // that differ when generating from nested vs outer workspaces.
        // We only hash the manifest contents and other data that affects dependency resolution.
        // See https://github.com/bazelbuild/rules_rust/issues/2125
        let normalized_splicing_metadata = serde_json::json!({
            // Direct packages don't contain workspace-specific paths
            "direct_packages": splicing_metadata.direct_packages,
            // Sort manifest contents without their labels to ensure consistent hashing
            "manifest_contents": splicing_metadata.manifests
                .values()
                .map(|m| serde_json::to_value(m).unwrap())
                .collect::<Vec<_>>(),
            // Cargo config doesn't contain workspace-specific paths
            "cargo_config": splicing_metadata.cargo_config,
        });
        
        hasher.update(Digest::compute_single_hash(
            &serde_json::to_string(&normalized_splicing_metadata).unwrap(),
            "splicing manifest",
        ));
        hasher.update(b"\0");
